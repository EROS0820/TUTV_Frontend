...
public final class RingtoneUtils {
    ...
    private static LinkedHashMap<String, Uri> getTone(@NonNull final Context context, final int type) {
       ...
       /* final RingtoneManager ringtoneManager = new RingtoneManager(context);
        ...
        Uri alarmSound1 = Uri.parse(ContentResolver.SCHEME_ANDROID_RESOURCE + "://" + context.getPackageName() +  "/raw/alarm1");
        Uri alarmSound2 = Uri.parse(ContentResolver.SCHEME_ANDROID_RESOURCE + "://" + context.getPackageName() +  "/raw/alarm2");
        Log.i("RingtoneUtils","alarmSound1 : "+alarmSound1);

        // alarmSoundListUri
        ArrayList<Uri> alarmSoundListUri = new ArrayList<Uri>();
        alarmSoundListUri.add(alarmSound1);
        alarmSoundListUri.add(alarmSound2);

        ArrayList<String> alarmSoundListString = new ArrayList<String>();
        alarmSoundListString.add("Alarm1");
        alarmSoundListString.add("Alarm2");

        for (int i=0; i < alarmSoundListUri.size(); i++){
//            ringToneList.put(alarmSoundListString.get(i),alarmSoundListUri.get(i) + R.raw.alarm1);
            ringToneList.put(alarmSoundListString.get(i),alarmSoundListUri.get(i));
            Log.i("RingtoneUtils","ringToneList size  : "+ringToneList.size());
        }
       /* ringToneList.put("alarm1",R.raw.alarm1.mp3);
        ringToneList.put("alarm2",R.raw.alarm2.mp3);*/
        return ringToneList;
    }
